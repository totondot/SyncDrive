...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // --- CORE NAVIGATION GROUP (Features 7, 11, then 6) ---

        // Setup Feature 7 (Routing & ETA) - Must be initialized before Feature 6
        tvRouteInfo = findViewById(R.id.tvRouteInfo)
        routeController = RouteController(mapView, tvRouteInfo)

        // Setup Feature 11 (Trip History) - Must be initialized before Feature 6
        btnTripHistory = findViewById(R.id.btnTripHistory)
        tripHistoryController = TripHistoryController(btnTripHistory)
        tripHistoryController.setupHistoryButton()

        // Setup Feature 6 (Set Destination) - Wires 7 and 11 together
        tvDestination = findViewById(R.id.tvDestination)
        navigationController = NavigationController(mapView, tvDestination) { destinationGeoPoint ->
            // 1. Draw the route (Feature 7)
            routeController.calculateAndDrawRoute(carCurrentLocation, destinationGeoPoint)

            // 2. Calculate the distance and save it to history (Feature 11)
            val distanceMeters = carCurrentLocation.distanceToAsDouble(destinationGeoPoint)
            val distanceKm = distanceMeters / 1000.0
            tripHistoryController.addTripToHistory(distanceKm)
        }
